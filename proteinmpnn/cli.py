#!/usr/bin/env python
"""
Command-line interface for ProteinMPNN.

This provides the same interface as the original protein_mpnn_run.py
but uses the new modular architecture.
"""

import argparse
import os
import sys
import time
import copy
import random
import numpy as np
import torch

from proteinmpnn.config.loaders import load_design_config, load_model
from proteinmpnn.data.parsers import parse_PDB
from proteinmpnn.data.featurize import featurize_batch
from proteinmpnn.data.datasets import StructureDataset, StructureDatasetPDB
from proteinmpnn.data.types import SamplingConfig
from proteinmpnn.utils.helpers import (
    scores_from_log_probs,
    sequence_to_string,
    parse_fasta,
    ALPHABET
)


def main(args):
    """Main entry point for ProteinMPNN inference."""

    # Set random seeds
    if args.seed:
        seed = args.seed
    else:
        seed = int(np.random.randint(0, high=999, size=1, dtype=int)[0])

    torch.manual_seed(seed)
    random.seed(seed)
    np.random.seed(seed)

    # Device setup
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

    # Validate CA-only and soluble model combination
    if args.ca_only and args.use_soluble_model:
        print("WARNING: CA-SolubleMPNN is not available yet")
        sys.exit()

    # Print settings
    print_all = args.suppress_print == 0

    if print_all:
        if args.ca_only:
            print("Using CA-ProteinMPNN!")
        if args.use_soluble_model:
            print("Using ProteinMPNN trained on soluble proteins only!")

    # Load model
    model, checkpoint_info = load_model(
        model_name=args.model_name,
        ca_only=args.ca_only,
        use_soluble_model=args.use_soluble_model,
        path_to_model_weights=args.path_to_model_weights,
        device=device,
        backbone_noise=args.backbone_noise
    )

    if print_all:
        print('-' * 40)
        print('Number of edges:', checkpoint_info['num_edges'])
        print(f'Training noise level: {checkpoint_info["noise_level"]}A')

    # Load design configuration
    design_config = load_design_config(
        chain_id_jsonl=args.chain_id_jsonl,
        fixed_positions_jsonl=args.fixed_positions_jsonl,
        omit_AA_jsonl=args.omit_AA_jsonl,
        tied_positions_jsonl=args.tied_positions_jsonl,
        pssm_jsonl=args.pssm_jsonl,
        bias_AA_jsonl=args.bias_AA_jsonl,
        bias_by_res_jsonl=args.bias_by_res_jsonl,
        verbose=print_all
    )

    # Prepare AA constraints
    omit_AAs_np = np.array([AA in args.omit_AAs for AA in ALPHABET]).astype(np.float32)
    bias_AAs_np = np.zeros(len(ALPHABET))
    if design_config.bias_AA_dict:
        for n, AA in enumerate(ALPHABET):
            if AA in design_config.bias_AA_dict:
                bias_AAs_np[n] = design_config.bias_AA_dict[AA]

    # Load structure
    if args.pdb_path:
        pdb_dict_list = parse_PDB(args.pdb_path, ca_only=args.ca_only)
        dataset_valid = StructureDatasetPDB(
            pdb_dict_list, truncate=None, max_length=args.max_length
        )

        all_chain_list = [
            item[-1:] for item in list(pdb_dict_list[0])
            if item[:9] == 'seq_chain'
        ]

        if args.pdb_path_chains:
            designed_chain_list = [str(item) for item in args.pdb_path_chains.split()]
        else:
            designed_chain_list = all_chain_list

        fixed_chain_list = [
            letter for letter in all_chain_list
            if letter not in designed_chain_list
        ]

        chain_id_dict = {
            pdb_dict_list[0]['name']: (designed_chain_list, fixed_chain_list)
        }
    else:
        dataset_valid = StructureDataset(
            args.jsonl_path, truncate=None, max_length=args.max_length, verbose=print_all
        )
        chain_id_dict = design_config.chain_id_dict

    # Setup output directories
    base_folder = args.out_folder
    if base_folder[-1] != '/':
        base_folder = base_folder + '/'

    os.makedirs(base_folder, exist_ok=True)
    os.makedirs(base_folder + 'seqs', exist_ok=True)

    if args.save_score:
        os.makedirs(base_folder + 'scores', exist_ok=True)
    if args.score_only:
        os.makedirs(base_folder + 'score_only', exist_ok=True)
    if args.conditional_probs_only:
        os.makedirs(base_folder + 'conditional_probs_only', exist_ok=True)
    if args.unconditional_probs_only:
        os.makedirs(base_folder + 'unconditional_probs_only', exist_ok=True)
    if args.save_probs:
        os.makedirs(base_folder + 'probs', exist_ok=True)

    # Parse temperatures
    temperatures = [float(item) for item in args.sampling_temp.split()]
    NUM_BATCHES = args.num_seq_per_target // args.batch_size
    BATCH_COPIES = args.batch_size

    if print_all:
        print('-' * 40)

    # Main inference loop
    start_time = time.time()
    total_residues = 0

    with torch.no_grad():
        for ix, protein in enumerate(dataset_valid):
            score_list = []
            global_score_list = []
            all_probs_list = []
            all_log_probs_list = []
            S_sample_list = []

            batch_clones = [copy.deepcopy(protein) for _ in range(BATCH_COPIES)]

            # Featurize
            batch = featurize_batch(
                batch_clones, device, chain_id_dict,
                design_config.fixed_positions_dict,
                design_config.omit_AA_dict,
                design_config.tied_positions_dict,
                design_config.pssm_dict,
                design_config.bias_by_res_dict,
                ca_only=args.ca_only
            )

            pssm_log_odds_mask = (batch.pssm_log_odds > args.pssm_threshold).float()
            name_ = batch_clones[0]['name']

            if args.score_only:
                _run_score_only(
                    args, model, batch, batch_clones, base_folder, device,
                    NUM_BATCHES, print_all, ALPHABET
                )

            elif args.conditional_probs_only:
                if print_all:
                    print(f'Calculating conditional probabilities for {name_}')

                conditional_probs_only_file = base_folder + '/conditional_probs_only/' + name_
                log_conditional_probs_list = []

                for j in range(NUM_BATCHES):
                    randn = torch.randn(batch.chain_M.shape, device=device)
                    log_conditional_probs = model.conditional_probs(
                        batch.X, batch.S, batch.mask,
                        batch.chain_M * batch.chain_M_pos,
                        batch.residue_idx, batch.chain_encoding_all,
                        randn, args.conditional_probs_only_backbone
                    )
                    log_conditional_probs_list.append(log_conditional_probs.cpu().numpy())

                concat_log_p = np.concatenate(log_conditional_probs_list, 0)
                mask_out = (batch.chain_M * batch.chain_M_pos * batch.mask)[0].cpu().numpy()
                np.savez(
                    conditional_probs_only_file,
                    log_p=concat_log_p,
                    S=batch.S[0].cpu().numpy(),
                    mask=batch.mask[0].cpu().numpy(),
                    design_mask=mask_out
                )

            elif args.unconditional_probs_only:
                if print_all:
                    print(f'Calculating sequence unconditional probabilities for {name_}')

                unconditional_probs_only_file = base_folder + '/unconditional_probs_only/' + name_
                log_unconditional_probs_list = []

                for j in range(NUM_BATCHES):
                    log_unconditional_probs = model.unconditional_probs(
                        batch.X, batch.mask, batch.residue_idx, batch.chain_encoding_all
                    )
                    log_unconditional_probs_list.append(log_unconditional_probs.cpu().numpy())

                concat_log_p = np.concatenate(log_unconditional_probs_list, 0)
                mask_out = (batch.chain_M * batch.chain_M_pos * batch.mask)[0].cpu().numpy()
                np.savez(
                    unconditional_probs_only_file,
                    log_p=concat_log_p,
                    S=batch.S[0].cpu().numpy(),
                    mask=batch.mask[0].cpu().numpy(),
                    design_mask=mask_out
                )

            else:
                # Standard sequence generation
                _run_generation(
                    args, model, batch, batch_clones, base_folder, device,
                    temperatures, NUM_BATCHES, BATCH_COPIES, print_all,
                    omit_AAs_np, bias_AAs_np, pssm_log_odds_mask,
                    design_config, seed, score_list, global_score_list,
                    all_probs_list, all_log_probs_list, S_sample_list
                )


def _run_score_only(args, model, batch, batch_clones, base_folder, device,
                    NUM_BATCHES, print_all, alphabet):
    """Run score-only mode."""
    alphabet_dict = {aa: i for i, aa in enumerate(alphabet)}

    loop_c = 0
    if args.path_to_fasta:
        fasta_names, fasta_seqs = parse_fasta(args.path_to_fasta, omit=["/"])
        loop_c = len(fasta_seqs)

    for fc in range(1 + loop_c):
        if fc == 0:
            structure_sequence_score_file = base_folder + '/score_only/' + batch_clones[0]['name'] + '_pdb'
        else:
            structure_sequence_score_file = base_folder + '/score_only/' + batch_clones[0]['name'] + f'_fasta_{fc}'

        native_score_list = []
        global_native_score_list = []

        if fc > 0:
            input_seq_length = len(fasta_seqs[fc - 1])
            S_input = torch.tensor(
                [alphabet_dict[AA] for AA in fasta_seqs[fc - 1]], device=device
            )[None, :].repeat(batch.X.shape[0], 1)
            batch.S[:, :input_seq_length] = S_input

        for j in range(NUM_BATCHES):
            randn = torch.randn(batch.chain_M.shape, device=device)
            log_probs = model(
                batch.X, batch.S, batch.mask,
                batch.chain_M * batch.chain_M_pos,
                batch.residue_idx, batch.chain_encoding_all, randn
            )
            mask_for_loss = batch.mask * batch.chain_M * batch.chain_M_pos
            scores = scores_from_log_probs(batch.S, log_probs, mask_for_loss)
            native_score_list.append(scores.cpu().data.numpy())
            global_scores = scores_from_log_probs(batch.S, log_probs, batch.mask)
            global_native_score_list.append(global_scores.cpu().data.numpy())

        native_score = np.concatenate(native_score_list, 0)
        global_native_score = np.concatenate(global_native_score_list, 0)

        ns_mean = native_score.mean()
        ns_std = native_score.std()
        global_ns_mean = global_native_score.mean()
        global_ns_std = global_native_score.std()

        seq_str = sequence_to_string(batch.S[0], batch.chain_M[0])
        np.savez(
            structure_sequence_score_file,
            score=native_score,
            global_score=global_native_score,
            S=batch.S[0].cpu().numpy(),
            seq_str=seq_str
        )

        if print_all:
            ns_mean_print = np.format_float_positional(np.float32(ns_mean), unique=False, precision=4)
            ns_std_print = np.format_float_positional(np.float32(ns_std), unique=False, precision=4)
            global_ns_mean_print = np.format_float_positional(np.float32(global_ns_mean), unique=False, precision=4)
            global_ns_std_print = np.format_float_positional(np.float32(global_ns_std), unique=False, precision=4)

            if fc == 0:
                print(f'Score for {batch_clones[0]["name"]} from PDB, mean: {ns_mean_print}, std: {ns_std_print}, '
                      f'sample size: {native_score.shape[0]}, global score, mean: {global_ns_mean_print}, '
                      f'std: {global_ns_std_print}')
            else:
                print(f'Score for {batch_clones[0]["name"]}_{fc} from FASTA, mean: {ns_mean_print}, std: {ns_std_print}, '
                      f'sample size: {native_score.shape[0]}, global score, mean: {global_ns_mean_print}, '
                      f'std: {global_ns_std_print}')


def _run_generation(args, model, batch, batch_clones, base_folder, device,
                    temperatures, NUM_BATCHES, BATCH_COPIES, print_all,
                    omit_AAs_np, bias_AAs_np, pssm_log_odds_mask,
                    design_config, seed, score_list, global_score_list,
                    all_probs_list, all_log_probs_list, S_sample_list):
    """Run standard sequence generation."""
    import subprocess

    name_ = batch_clones[0]['name']

    # Get native score first
    randn_1 = torch.randn(batch.chain_M.shape, device=device)
    log_probs = model(
        batch.X, batch.S, batch.mask,
        batch.chain_M * batch.chain_M_pos,
        batch.residue_idx, batch.chain_encoding_all, randn_1
    )
    mask_for_loss = batch.mask * batch.chain_M * batch.chain_M_pos
    scores = scores_from_log_probs(batch.S, log_probs, mask_for_loss)
    native_score = scores.cpu().data.numpy()
    global_scores = scores_from_log_probs(batch.S, log_probs, batch.mask)
    global_native_score = global_scores.cpu().data.numpy()

    # Output files
    ali_file = base_folder + '/seqs/' + name_ + '.fa'
    score_file = base_folder + '/scores/' + name_ + '.npz'
    probs_file = base_folder + '/probs/' + name_ + '.npz'

    if print_all:
        print(f'Generating sequences for: {name_}')

    t0 = time.time()

    with open(ali_file, 'w') as f:
        for temp in temperatures:
            for j in range(NUM_BATCHES):
                randn_2 = torch.randn(batch.chain_M.shape, device=device)

                # Create sampling config
                sampling_config = SamplingConfig(
                    temperature=temp,
                    omit_AAs_np=omit_AAs_np,
                    bias_AAs_np=bias_AAs_np,
                    chain_M_pos=batch.chain_M_pos,
                    omit_AA_mask=batch.omit_AA_mask,
                    pssm_coef=batch.pssm_coef,
                    pssm_bias=batch.pssm_bias,
                    pssm_multi=args.pssm_multi,
                    pssm_log_odds_flag=bool(args.pssm_log_odds_flag),
                    pssm_log_odds_mask=pssm_log_odds_mask,
                    pssm_bias_flag=bool(args.pssm_bias_flag),
                    bias_by_res=batch.bias_by_res
                )

                if design_config.tied_positions_dict is None:
                    sample_dict = model.sample(
                        batch.X, randn_2, batch.S, batch.chain_M,
                        batch.chain_encoding_all, batch.residue_idx,
                        mask=batch.mask, config=sampling_config
                    )
                else:
                    sample_dict = model.tied_sample(
                        batch.X, randn_2, batch.S, batch.chain_M,
                        batch.chain_encoding_all, batch.residue_idx,
                        mask=batch.mask, config=sampling_config,
                        tied_pos=batch.tied_positions[0],
                        tied_beta=batch.tied_beta
                    )

                S_sample = sample_dict["S"]

                # Compute scores
                log_probs = model(
                    batch.X, S_sample, batch.mask,
                    batch.chain_M * batch.chain_M_pos,
                    batch.residue_idx, batch.chain_encoding_all,
                    randn_2, use_input_decoding_order=True,
                    decoding_order=sample_dict["decoding_order"]
                )

                mask_for_loss = batch.mask * batch.chain_M * batch.chain_M_pos
                scores = scores_from_log_probs(S_sample, log_probs, mask_for_loss)
                scores = scores.cpu().data.numpy()

                global_scores = scores_from_log_probs(S_sample, log_probs, batch.mask)
                global_scores = global_scores.cpu().data.numpy()

                all_probs_list.append(sample_dict["probs"].cpu().data.numpy())
                all_log_probs_list.append(log_probs.cpu().data.numpy())
                S_sample_list.append(S_sample.cpu().data.numpy())

                for b_ix in range(BATCH_COPIES):
                    chain_info = batch.chain_info_list[b_ix]
                    masked_chain_length_list = chain_info.masked_chain_lengths
                    masked_list = chain_info.masked_list

                    seq_recovery_rate = torch.sum(
                        torch.sum(
                            torch.nn.functional.one_hot(batch.S[b_ix], 21) *
                            torch.nn.functional.one_hot(S_sample[b_ix], 21),
                            axis=-1
                        ) * mask_for_loss[b_ix]
                    ) / torch.sum(mask_for_loss[b_ix])

                    seq = sequence_to_string(S_sample[b_ix], batch.chain_M[b_ix])
                    score = scores[b_ix]
                    score_list.append(score)
                    global_score = global_scores[b_ix]
                    global_score_list.append(global_score)
                    native_seq = sequence_to_string(batch.S[b_ix], batch.chain_M[b_ix])

                    # First sample header
                    if b_ix == 0 and j == 0 and temp == temperatures[0]:
                        # Reorder sequence by chain
                        start = 0
                        end = 0
                        list_of_AAs = []
                        for mask_l in masked_chain_length_list:
                            end += mask_l
                            list_of_AAs.append(native_seq[start:end])
                            start = end
                        native_seq = "".join(list(np.array(list_of_AAs)[np.argsort(masked_list)]))

                        l0 = 0
                        for mc_length in list(np.array(masked_chain_length_list)[np.argsort(masked_list)])[:-1]:
                            l0 += mc_length
                            native_seq = native_seq[:l0] + '/' + native_seq[l0:]
                            l0 += 1

                        sorted_masked_chain_letters = np.argsort(chain_info.masked_list)
                        print_masked_chains = [chain_info.masked_list[i] for i in sorted_masked_chain_letters]
                        sorted_visible_chain_letters = np.argsort(chain_info.visible_list)
                        print_visible_chains = [chain_info.visible_list[i] for i in sorted_visible_chain_letters] if chain_info.visible_list else []

                        native_score_print = np.format_float_positional(
                            np.float32(native_score.mean()), unique=False, precision=4
                        )
                        global_native_score_print = np.format_float_positional(
                            np.float32(global_native_score.mean()), unique=False, precision=4
                        )

                        script_dir = os.path.dirname(os.path.realpath(__file__))
                        try:
                            commit_str = subprocess.check_output(
                                f'git --git-dir {os.path.dirname(script_dir)}/.git rev-parse HEAD',
                                shell=True, stderr=subprocess.DEVNULL
                            ).decode().strip()
                        except subprocess.CalledProcessError:
                            commit_str = 'unknown'

                        print_model_name = 'CA_model_name' if args.ca_only else 'model_name'
                        f.write(
                            f'>{name_}, score={native_score_print}, global_score={global_native_score_print}, '
                            f'fixed_chains={print_visible_chains}, designed_chains={print_masked_chains}, '
                            f'{print_model_name}={args.model_name}, git_hash={commit_str}, seed={seed}\n'
                            f'{native_seq}\n'
                        )

                    # Reorder generated sequence
                    start = 0
                    end = 0
                    list_of_AAs = []
                    for mask_l in masked_chain_length_list:
                        end += mask_l
                        list_of_AAs.append(seq[start:end])
                        start = end

                    seq = "".join(list(np.array(list_of_AAs)[np.argsort(masked_list)]))
                    l0 = 0
                    for mc_length in list(np.array(masked_chain_length_list)[np.argsort(masked_list)])[:-1]:
                        l0 += mc_length
                        seq = seq[:l0] + '/' + seq[l0:]
                        l0 += 1

                    score_print = np.format_float_positional(np.float32(score), unique=False, precision=4)
                    global_score_print = np.format_float_positional(np.float32(global_score), unique=False, precision=4)
                    seq_rec_print = np.format_float_positional(
                        np.float32(seq_recovery_rate.detach().cpu().numpy()), unique=False, precision=4
                    )
                    sample_number = j * BATCH_COPIES + b_ix + 1
                    f.write(
                        f'>T={temp}, sample={sample_number}, score={score_print}, '
                        f'global_score={global_score_print}, seq_recovery={seq_rec_print}\n{seq}\n'
                    )

        if args.save_score:
            np.savez(
                score_file,
                score=np.array(score_list, np.float32),
                global_score=np.array(global_score_list, np.float32)
            )

        if args.save_probs:
            all_probs_concat = np.concatenate(all_probs_list)
            all_log_probs_concat = np.concatenate(all_log_probs_list)
            S_sample_concat = np.concatenate(S_sample_list)
            np.savez(
                probs_file,
                probs=np.array(all_probs_concat, np.float32),
                log_probs=np.array(all_log_probs_concat, np.float32),
                S=np.array(S_sample_concat, np.int32),
                mask=mask_for_loss.cpu().data.numpy(),
                chain_order=[info.letter_list for info in batch.chain_info_list]
            )

        t1 = time.time()
        dt = round(float(t1 - t0), 4)
        num_seqs = len(temperatures) * NUM_BATCHES * BATCH_COPIES
        total_length = batch.X.shape[1]

        if print_all:
            print(f'{num_seqs} sequences of length {total_length} generated in {dt} seconds')


def run():
    """Entry point for CLI."""
    argparser = argparse.ArgumentParser(formatter_class=argparse.ArgumentDefaultsHelpFormatter)

    argparser.add_argument("--suppress_print", type=int, default=0, help="0 for False, 1 for True")

    argparser.add_argument("--ca_only", action="store_true", default=False,
                          help="Parse CA-only structures and use CA-only models")
    argparser.add_argument("--path_to_model_weights", type=str, default="",
                          help="Path to model weights folder")
    argparser.add_argument("--model_name", type=str, default="v_48_020",
                          help="ProteinMPNN model name: v_48_002, v_48_010, v_48_020, v_48_030")
    argparser.add_argument("--use_soluble_model", action="store_true", default=False,
                          help="Use ProteinMPNN weights trained on soluble proteins only")

    argparser.add_argument("--seed", type=int, default=0,
                          help="If set to 0 then a random seed will be picked")

    argparser.add_argument("--save_score", type=int, default=0,
                          help="0 for False, 1 for True; save score=-log_prob to npy files")
    argparser.add_argument("--save_probs", type=int, default=0,
                          help="0 for False, 1 for True; save MPNN predicted probabilities")

    argparser.add_argument("--score_only", type=int, default=0,
                          help="0 for False, 1 for True; score input backbone-sequence pairs")
    argparser.add_argument("--path_to_fasta", type=str, default="",
                          help="Score provided input sequence in a fasta format")

    argparser.add_argument("--conditional_probs_only", type=int, default=0,
                          help="0 for False, 1 for True; output conditional probabilities")
    argparser.add_argument("--conditional_probs_only_backbone", type=int, default=0,
                          help="0 for False, 1 for True; output p(s_i given backbone)")
    argparser.add_argument("--unconditional_probs_only", type=int, default=0,
                          help="0 for False, 1 for True; output unconditional probabilities")

    argparser.add_argument("--backbone_noise", type=float, default=0.00,
                          help="Standard deviation of Gaussian noise to add to backbone atoms")
    argparser.add_argument("--num_seq_per_target", type=int, default=1,
                          help="Number of sequences to generate per target")
    argparser.add_argument("--batch_size", type=int, default=1,
                          help="Batch size")
    argparser.add_argument("--max_length", type=int, default=200000,
                          help="Max sequence length")
    argparser.add_argument("--sampling_temp", type=str, default="0.1",
                          help="Sampling temperature(s), space-separated")

    argparser.add_argument("--out_folder", type=str,
                          help="Path to output folder")
    argparser.add_argument("--pdb_path", type=str, default='',
                          help="Path to a single PDB to be designed")
    argparser.add_argument("--pdb_path_chains", type=str, default='',
                          help="Chains to design for a single PDB")
    argparser.add_argument("--jsonl_path", type=str,
                          help="Path to JSONL with parsed PDBs")
    argparser.add_argument("--chain_id_jsonl", type=str, default='',
                          help="Path to chain ID configuration")
    argparser.add_argument("--fixed_positions_jsonl", type=str, default='',
                          help="Path to fixed positions configuration")
    argparser.add_argument("--omit_AAs", type=str, default='X',
                          help="Amino acids to omit in generated sequence")
    argparser.add_argument("--bias_AA_jsonl", type=str, default='',
                          help="Path to AA composition bias configuration")

    argparser.add_argument("--bias_by_res_jsonl", default='',
                          help="Path to per-residue bias configuration")
    argparser.add_argument("--omit_AA_jsonl", type=str, default='',
                          help="Path to per-position omit AA configuration")
    argparser.add_argument("--pssm_jsonl", type=str, default='',
                          help="Path to PSSM configuration")
    argparser.add_argument("--pssm_multi", type=float, default=0.0,
                          help="PSSM multiplier [0.0, 1.0]")
    argparser.add_argument("--pssm_threshold", type=float, default=0.0,
                          help="PSSM threshold")
    argparser.add_argument("--pssm_log_odds_flag", type=int, default=0,
                          help="0 for False, 1 for True")
    argparser.add_argument("--pssm_bias_flag", type=int, default=0,
                          help="0 for False, 1 for True")

    argparser.add_argument("--tied_positions_jsonl", type=str, default='',
                          help="Path to tied positions configuration")

    args = argparser.parse_args()
    main(args)


if __name__ == "__main__":
    run()
